...
class ConversationMessage(BaseModel):
    ...
    @classmethod
    def create_user_message(cls, content: str, message_id: Optional[str] = None) -> "ConversationMessage":
    ...
    @classmethod
    def create_assistant_message(cls, content: str, message_id: Optional[str] = None) -> "ConversationMessage":
...
class ConversationTurn(BaseModel):
    ...
    def add_tool_call(self, function_name: str, arguments: Dict[str, Any], tool_call_id: str) -> ToolCall:
    ...
    def add_tool_result(self, tool_call_id: str, function_name: str, result: str, success: bool = True) -> ToolResult:
    ...
    def complete_turn(self, status: str = "completed", error_message: Optional[str] = None):
...
class ConversationSummary(BaseModel):
    ...
    most_used_tools: List[Dict[str, Union[str, int]]]  # [{"name": "tool_name", "count": 5}]
...
class ConversationDebugData(BaseModel):
    ...
    def start_new_turn(self, user_message: Optional[ConversationMessage] = None) -> ConversationTurn:
    ...
    def update_summary(self):
        ...
        total_messages = sum(1 for turn in self.turns if turn.user_message) + sum(1 for turn in self.turns if turn.assistant_response)
        ...
        avg_turn_duration_ms = sum(turn.duration_ms for turn in completed_turns) / len(completed_turns) if completed_turns else None
        
        # Calculate total conversation duration
        start_time = self.turns[0].start_time if self.turns else self.created_at
        last_completed_turn = next((turn for turn in reversed(self.turns) if turn.end_time), None)
        ...
        for turn in self.turns:
            for tool_call in turn.tool_calls:
                tool_counts[tool_call.function_name] = tool_counts.get(tool_call.function_name, 0) + 1
...
class ConversationEvent(BaseModel):
    ...
    def turn_start(cls, session_id: str, turn: ConversationTurn) -> "ConversationEvent":
        ...
        return cls(
            ...
            payload={
                ...
                "status": turn.status,
                "user_message": turn.user_message.model_dump() if turn.user_message else None,
    ...
    @classmethod  
    def turn_update(cls, session_id: str, turn: ConversationTurn, update_type: str) -> "ConversationEvent":
    ...
    @classmethod
    def turn_complete(cls, session_id: str, turn: ConversationTurn) -> "ConversationEvent":
    ...
    @classmethod
    def full_state(cls, session_id: str, conversation: ConversationDebugData) -> "ConversationEvent":
        ...
        return cls(
            ...
            payload={
                ...
                "summary": conversation.summary.model_dump() if conversation.summary else None
            }
        )