...
async def get_current_shop_config_data(shop_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"HTTP error fetching shop config: {e.response.status_code} - {e.response.text}")
        raise ValueError(f"Failed to fetch shop configuration: HTTP {e.response.status_code}")
    except Exception as e:
...
async def patch_shop_config(
    ...
    try:
        ...
        logger.error(f"HTTP error calling patch_shop_config: {e.response.status_code} - {e.response.text}")
        return {
                "status": "failure",
